...
## Approaches & Discussion
### Rachel
- **Understand**: We're given the root of a binary tree and need to verify if it's a valid BST. The key insight is that every node in the left subtree must be smaller than the root, and every node in the right subtree must be larger.
